...
class	Matrix
{
	...
	public:
						Matrix(void) : _nbrColumns(0), _nbrLines(0) {}
		virtual				~Matrix(void) {}
		...
		Matrix<T>			getAdjugate(void) const { return getComatrix().getTranspose(); }
		std::vector<Complex>		getEigenValues(void) const;
		std::vector<Vector<Complex>>	getEigenVectors(void) const;

		void				display(void) const;
		bool				isSquare(void) const { return _nbrLines == _nbrColumns; }
		bool				isDiagonal(void) const;
		bool				isInversible(void) const { return getDeterminant() && !isNull(); }
